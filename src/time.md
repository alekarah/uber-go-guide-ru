...
Поэтому всегда используйте пакет [`"time"`] при работе со временем, потому что он помогает корректно обрабатывать эти типичные заблуждения безопасным и точным образом.
...
Возвращаясь к примеру добавления 24 часов к моменту времени, метод, который мы используем для добавления времени, зависит от намерения. Если мы хотим то же время суток, но на следующий календарный день, мы должны использовать [`Time.AddDate`]. Однако, если мы хотим момент времени, ровно через 24 часа после предыдущего момента, мы должны использовать [`Time.Add`].
...
Используйте `time.Time` и `time.Duration` при взаимодействии с внешними системами, когда это возможно. Например:
...
Если использовать `time.Time` в этих взаимодействиях невозможно, используйте `string` и форматируйте временные метки согласно [RFC 3339]. Этот формат используется по умолчанию методом [`Time.UnmarshalText`] и доступен для использования в `Time.Format` и `time.Parse` через [`time.RFC3339`].

  [`Time.UnmarshalText`]: https://pkg.go.dev/time#Time.UnmarshalText
  [`time.RFC3339`]: https://pkg.go.dev/time#RFC3339

Хотя на практике это обычно не проблема, учтите, что пакет `"time"` не поддерживает парсинг временных меток с високосными секундами ([8728]), и также не учитывает високосные секунды в вычислениях ([15190]). Если вы сравниваете два момента времени, разница не будет включать високосные секунды, которые могли произойти между этими двумя моментами.
