<!--
  ...
  Для внесения изменений редактируйте файлы в директории "src-ru".
-->
...
- [Руководства](#руководства)
  - [Указатели на интерфейсы](#pointers-to-interfaces)
  - [Проверка соответствия интерфейсу](#verify-interface-compliance)
  - [Приемники и интерфейсы](#receivers-and-interfaces)
  - [Мьютексы с нулевым значением валидны](#zero-value-mutexes-are-valid)
  - [Копирование слайсов и map на границах](#copy-slices-and-maps-at-boundaries)
  - [Defer для очистки ресурсов](#defer-to-clean-up)
  - [Размер канала — один или ноль](#channel-size-is-one-or-none)
  - [Начинайте enum с единицы](#start-enums-at-one)
  - [Используйте `"time"` для работы со временем](#use-time-to-handle-time)
  - [Ошибки](#ошибки)
    - [Типы ошибок](#error-types)
    - [Оборачивание ошибок](#error-wrapping)
    - [Именование ошибок](#error-naming)
    - [Обрабатывайте ошибки только один раз](#handle-errors-once)
  - [Обрабатывайте ошибки приведения типов](#handle-type-assertion-failures)
  - [Не используйте panic](#%D0%BD%D0%B5-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D1%83%D0%B9%D1%82%D0%B5-panic)
  - [Используйте go.uber.org/atomic](#use-gouberorgatomic)
  - [Избегайте изменяемых глобальных переменных](#avoid-mutable-globals)
  - [Избегайте встраивания типов в публичные структуры](#avoid-embedding-types-in-public-structs)
  - [Избегайте использования встроенных имён](#avoid-using-built-in-names)
  - [Избегайте `init()`](#avoid-init)
  - [Выход из программы в Main](#exit-in-main)
    - [Выходите один раз](#exit-once)
  - [Используйте теги полей в маршалируемых структурах](#use-field-tags-in-marshaled-structs)
  - [Не запускайте горутины без контроля](#dont-fire-and-forget-goroutines)
    - [Ожидайте завершения горутин](#wait-for-goroutines-to-exit)
    - [Никаких горутин в `init()`](#no-goroutines-in-init)
- [Производительность](#performance)
  - [Предпочитайте strconv вместо fmt](#prefer-strconv-over-fmt)
  - [Избегайте повторных преобразований строк в байты](#avoid-repeated-string-to-byte-conversions)
  - [Предпочитайте указывать ёмкость контейнеров](#prefer-specifying-container-capacity)
- [Стиль](#стиль)
  - [Избегайте слишком длинных строк](#avoid-overly-long-lines)
  - [Будьте последовательны](#be-consistent)
  - [Группируйте похожие объявления](#group-similar-declarations)
  - [Порядок групп импортов](#import-group-ordering)
  - [Имена пакетов](#package-names)
  - [Имена функций](#function-names)
  - [Псевдонимы импортов](#import-aliasing)
  - [Группировка и порядок функций](#function-grouping-and-ordering)
  - [Уменьшайте вложенность](#reduce-nesting)
  - [Ненужный else](#unnecessary-else)
  - [Объявления переменных верхнего уровня](#top-level-variable-declarations)
  - [Префикс _ для неэкспортируемых глобальных переменных](#prefix-unexported-globals-with-_)
  - [Встраивание в структуры](#embedding-in-structs)
  - [Объявления локальных переменных](#local-variable-declarations)
  - [nil — валидный слайс](#nil-is-a-valid-slice)
  - [Уменьшайте область видимости переменных](#reduce-scope-of-variables)
  - [Избегайте голых параметров](#avoid-naked-parameters)
  - [Используйте сырые строковые литералы для избежания экранирования](#use-raw-string-literals-to-avoid-escaping)
  - [Инициализация структур](#инициализация-структур)
    - [Используйте имена полей при инициализации структур](#use-field-names-to-initialize-structs)
    - [Опускайте поля с нулевыми значениями в структурах](#omit-zero-value-fields-in-structs)
    - [Используйте `var` для структур с нулевыми значениями](#use-var-for-zero-value-structs)
    - [Инициализация ссылок на структуры](#initializing-struct-references)
  - [Инициализация map](#initializing-maps)
  - [Форматируйте строки вне Printf](#format-strings-outside-printf)
  - [Именование Printf-подобных функций](#naming-printf-style-functions)
- [Паттерны](#паттерны)
  - [Табличные тесты](#test-tables)
...
Это руководство было изначально создано [Prashant Varanasi](https://github.com/prashantv) и [Simon Newton](https://github.com/nomis52) как способ помочь коллегам быстрее освоить Go. С годами оно дорабатывалось на основе отзывов других разработчиков.
...
Код, работающий в production, должен избегать panic. Panic — это основная причина [каскадных сбоев](https://en.wikipedia.org/wiki/Cascading_failure). Если возникает ошибка, функция должна вернуть ошибку и позволить вызывающей стороне решить, как её обработать.
...
is not a good way to exit programs, please [don't panic](#%D0%BD%D0%B5-%D0%B8%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D1%83%D0%B9%D1%82%D0%B5-panic).)
