...
Это подходит, когда сообщение исходной ошибки
...
сопоставить и извлечь исходную причину.

- Используйте `%w`, если вызывающая сторона должна иметь доступ к исходной ошибке.
...
При добавлении контекста к возвращаемым ошибкам держите контекст кратким и избегайте
фраз вроде "failed to", которые констатируют очевидное и накапливаются по мере того,
...
Получив ошибку, вызывающая сторона может обработать её
несколькими способами в зависимости от того, что известно об ошибке.
...
потому что вызывающие стороны выше по стеку тоже могут обработать эту ошибку.
...
1. Быть полностью детерминированным, независимо от окружения и контекста вызова.
2. Не зависеть от порядка выполнения других функций `init()` и их побочных эффектов. Хотя порядок `init()` хорошо известен, код может измениться, и тогда зависимости между функциями `init()` сделают код хрупким и подверженным ошибкам.
3. Не обращаться к глобальному состоянию или состоянию окружения и не изменять их: информации о машине, переменных окружения, рабочей директории, аргументах и входных данных программы и т.д.
4. Не выполнять I/O, включая файловую систему, сеть и системные вызовы.

Код, который не может удовлетворить этим требованиям, скорее всего должен быть вспомогательной функцией, вызываемой как часть `main()` (или в другом месте жизненного цикла программы), или быть написан как часть самой `main()`. В частности, библиотеки, предназначенные для использования другими программами, должны стараться быть полностью детерминированными и не творить "магию init".
